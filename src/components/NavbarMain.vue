...
<template>
  <header>
    <nav class="flex items-center justify-between p-6 lg:px-8" aria-label="Global">
        ...
        <!-- <div class="flex gap-4">
          <li>
            <a href="https://www.facebook.com/" target="_blank" rel="noopener">
              <img src="@/assets/icons/fb-icon.svg" alt="facebook" class="h-6" />
            </a>
          </li>
          <li>
            <a href="https://www.instagram.com/" target="_blank" rel="noopener">
              <img src="@/assets/icons/ig-icon.svg" alt="instagram" class="h-6" />
            </a>
          </li>
        </div> -->
      </ul>
      <ul :class="isOpen ? 'translate-x-0' : 'translate-x-full'" class="lg:hidden fixed top-0 right-0 w-64 h-full bg-white shadow-lg text-gray-500 font-urbanist text-[1rem] transition-transform transform duration-300 ease-in-out z-40 p-6 flex flex-col gap-1 font-semibold">
        <li class="py-2 mt-12">
          <router-link to="/" class="block text-gray-800 hover:opacity-50 p-3">Strona główna</router-link>
        </li>
        <li class="py-2">
          <router-link to="/o-nas" class="block text-gray-800 transition-all duration-400 hover:opacity-50 p-3 ">O klubie</router-link>
        </li>
        <li class="py-2">
          <router-link to="/zawodnicy" class="block text-gray-800 hover:opacity-50 p-3">Zawodnicy</router-link>
        </li>
        <li class="py-2">
      ...
      <div v-if="isOpen"
      @click="isOpen = false" class="fixed inset-0 bg-white/10 backdrop-blur-lg backdrop-saturate-150 border border-white/20 shadow-lg p-6 rounded-xl z-30 lg:gidden"></div>
    </nav>
  </header>
</template>
